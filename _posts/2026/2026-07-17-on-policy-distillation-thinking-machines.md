---
title: "You Have Been Doing Distillation Wrong: Off-Policy vs On-Policy"
excerpt: "Most practitioners distill by training on teacher outputs — that's off-policy distillation, and it has a fundamental flaw. On-policy distillation fixes it at 9-30x lower compute than RL. Here's exactly why."
date: 2026-07-17
tags:
- llm
- finetuning
...
# You Have Been Doing Distillation Wrong: Off-Policy vs On-Policy
...
Thinking Machines Lab published a paper in October 2025 showing they trained a 0.5B model to match a 1.5B teacher's performance on math reasoning at **9–30x lower compute than RL**, just by switching from off-policy to on-policy. The training loop fits in 10 lines of PyTorch. This post explains why the switch matters and exactly what changes in the code.

---

## Table of Contents
1. [What Off-Policy Distillation Actually Is](#what-off-policy-distillation-actually-is)
2. [The Compounding Error Problem](#the-compounding-error-problem)
3. [What On-Policy Distillation Changes](#what-on-policy-distillation-changes)
4. [The Chess Coach Analogy](#the-chess-coach-analogy)
5. [The Loss Function: Reverse KL](#the-loss-function-reverse-kl)
6. [The Code: 10 Lines That Change Everything](#the-code-10-lines-that-change-everything)
7. [The Numbers from Thinking Machines](#the-numbers-from-thinking-machines)
8. [Run It Yourself: Colab Demo](#run-it-yourself-colab-demo)
9. [Summary](#summary)
...
This is **off-policy** distillation. The word "off-policy" means: the data you train on was generated by a different policy than the one you are training. The teacher generated the trajectories. The student just imitates them.
...
Thinking Machines uses an analogy in their paper that makes the intuition immediate.

**Off-policy distillation is watching a grandmaster play.** You observe extremely strong moves, played from board states a grandmaster reaches because they're a grandmaster. As a club player, you will rarely find yourself in those positions. What you observe is high quality but useless because it is in the wrong context.

**On-policy distillation is a coach watching you play, grading every move.** You play your own game. After each move the coach says: "That was a blunder — you should have castled here." You make mistakes the grandmaster never makes, and the coach grades those exact mistakes. You learn to correct yourself from the positions you actually reach.

If you have used chess.com's game analysis, you know exactly what this feels like. Every move is coloured from brilliant (blue) to blunder (red). That colour is the per-token KL — applied to every token your model generates.
...
Notice the two student forward passes. The first is inside `generate()` — autoregressive, token by token, no grad. It determines *what* tokens to produce. The second is inside `compute_token_logprobs` — parallel over the full sequence, with grad. It computes the log-probabilities the optimizer differentiates through. Sampling is not differentiable, so you need this second pass to get gradients.
...
**[Open in Google Colab →](https://colab.research.google.com/github/saraswatmks/saraswatmks.github.com/blob/master/notebooks/on_policy_distillation_colab.ipynb)**

---

## Summary

- **Off-policy distillation** (standard SFT on teacher outputs) trains the student in the teacher's world. The student never sees its own mistakes. Compounding error means every wrong token at inference takes the student further from anything it was trained on.

- **On-policy distillation** generates rollouts from the student and uses the teacher only to score them token by token. The training distribution matches the inference distribution. The compounding error problem disappears.

- **The loss is reverse KL**: $\mathbb{E}_{x \sim \text{student}}[\log P_\text{student}(x) - \log P_\text{teacher}(x)]$ per token. Mode-seeking, dense ($O(N)$ bits per episode vs $O(1)$ for RL), and contains a built-in entropy regulariser.

- **Thinking Machines hit 70% AIME'24 at ~1,800 GPU hours** where RL needed 17,920. The same method recovered instruction-following after catastrophic forgetting without touching domain data.

- **The uncomfortable implication**: if on-policy distillation copies what RL learns at 1/10th the compute, the question is how much of RL's expensive search is genuinely necessary versus just being the expensive prerequisite for cheap distillation to finish the job.

Did you find this post useful? I am curious to hear from you in comments below.

---

*Tags: [finetuning](https://saraswatmks.github.io/tags/#finetuning) [llm](https://saraswatmks.github.io/tags/#llm) [rlhf](https://saraswatmks.github.io/tags/#rlhf)*
